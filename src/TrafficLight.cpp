...
void TrafficLight::cycleThroughPhases()
{
    ...
    // Set a new cycle duration value between 4 and 6 seconds
    auto cycleDuration = std::chrono::seconds(rand() % 3 + 4);
    while(true) {
        // Sleep to reduce CPU usage
        std::this_thread::sleep_for(std::chrono::milliseconds(1));

        // Compute time since last update
        auto timeSinceLastUpdate = std::chrono::system_clock::now() - timeLastUpdate;
        if (timeSinceLastUpdate >= cycleDuration) {
            ...
            // TODO(thosio): Send update to message queue using move semantics
